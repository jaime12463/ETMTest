...
export const AvisoPlantilla= ({tipo,titulo,mensaje,dataCy}:TAviso)=>{
    let icono: any = null;
    const classes = useEstilos({tipo, conBotones:  false});
		switch (tipo) {
			case 'error':
				icono = (
					<CheckRedondoIcon
...
export const AvisoError=(props:TAvisoTemplate )=>{
    ...
    return (
        <AvisoPlantilla tipo='error' {...props}/>
    )
};



export const AvisoDeshacer=({titulo,mensaje,dataCy, acciones}:TAvisoTemplate & TAvisoAcciones )=>{
