...
*/
export const useObtenerTiposPedidoSegunConfiguracion = (propiedad:'validaPresupuesto' | 'esValorizado'  | 'esMandatorio' | 'habilitaPromocion', valor: any) => {
	const {tipoPedidos} = useObtenerConfiguracion();
	const obtenerTiposPedidoSegunConfiguracion = useCallback(() => {
		return tipoPedidos.filter( tipoPedido => tipoPedido[propiedad]===valor).map(tipoPedido=> tipoPedido.codigo);
	}, [tipoPedidos]);
