...
export class ProductosDelPedidoFiltrados {
	...
	static obtenerListaIndex(
		...
		const condicionesDeFiltrado = new ProductosDelPedidoFiltrosCondiciones(
			configuracion,
			tipoDePago
		);
...
export class ProductosDelPedidoFiltrosCondiciones {
	...
	private queNOSeaPromoPush = (producto: TProductoPedido) =>
		!producto.promoPush;

	private queSeaDelTipoDePago = (
		producto: TProductoPedido,
	...
	private queNoSeaUnImplicitoAgregado = (producto: TProductoPedido) =>
		producto.codigoPromo == undefined;
	...
	private sinDecuentosAutomaticos = (producto: TProductoPedido) =>
		this.default(producto) && this.queNOSeaDescuentoAutomatico(producto);
