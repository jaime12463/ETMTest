...
export type TProductosPedidoIndex =  Record<number, TProductoPedido & {aplicado:number }>;
 ...
 */
export const obtenerlistaPromocionesVigentes = ( cliente:TCliente, listaPromociones:TListaPromoOngoing):TListaPromoOngoingConIndices =>
{
    let listaPromos:TListaPromoOngoing={};
    const fDispositivo=fechaDispositivo();
    const indicePorTipoId:string[]=[];
    cliente.promocionesHabilitadas?.forEach((promo) => {
        if (promo.pomocionesDisponibles>0 
            &&  fechaDentroDelRango(fDispositivo, listaPromociones[promo.idPromocion].inicioVigenciaPromocion, listaPromociones[promo.idPromocion].finVigenciaPromocion)
           )
            listaPromos={
                ...listaPromos,
                [promo.idPromocion]: listaPromociones[promo.idPromocion]
            };
            indicePorTipoId.push(`${listaPromociones[promo.idPromocion].aplicacion}${promo.idPromocion}`)
    })
    return {
        lista:listaPromos,
 ...
 * @returns {TListaPromoOngoing}
 */
export const obtenerPromocionesOngoingAplicables = (cliente:TCliente, productosPedidos: TProductosPedidoIndex, listaPromocionesVigentes:TListaPromoOngoingConIndices)  =>
{
    ...
    for ( let clave of listaPromocionesVigentes.indexPorTipoId )
    {
        ...
        const promo:TPromoOngoing=listaPromocionesVigentes.lista[Number(promocionID)];
        let materialesRequisitosVerificados: TPromoOngoingMaterialesRequisitosVerificados[]=[];
        let multiplo=[];
        let conector:any='Y';
        
        for ( let i=0; i<promo.requisitos.length; i++) // verificación de requisistos de la promo
        {
            materialesRequisitosVerificados.push(verificarRequisito(promo.requisitos[i],productosPedidos, productosUsadosEnOtrasPromos ));
            multiplo.push(materialesRequisitosVerificados[materialesRequisitosVerificados.length-1].multiplo);
        ...
        if (sonValidosLosRequisitos)
        {
             ...
             {
                if (promo.aplicacion==EFormaDeAplicacion.Automatica) //realizar asignación automática
                {
                    /**
                        ...
                        secuencias:[
                            {
                                ...
                                materialesBeneficio:[grupoDeBeneficios[0].secuencias[0].materialesBeneficio[0]]
                            }
                        ]
                    });
...
export const aplicarAutomaticamenteProductos= (listaMateriales:number[],cantidad:number):TPromoOngoingListaProductosAplicados[] =>
{
 ...
 */
const obtenerProductosDelPedidoIndex=  ( productosPedidos: TProductoPedido[], tipoPago: ETiposDePago, aplicadas: number[]) :TProductosPedidoIndex =>
{
    return productosPedidos.reduce(
        (productosPedidoIndex: TProductosPedidoIndex, producto: TProductoPedido ) => {
 ...
 */
const verificarRequisito = ( requisito:TPromoOngoingRequisitos,  productosIndex :TProductosPedidoIndex, productosUsadosEnOtrasPromos:TProductosUsadosEnOtrasPromos):TPromoOngoingMaterialesRequisitosVerificados =>
{
    let multiplo=0;
    ...
    requisito.materiales.forEach( (material) => {
        ...
        {
            TotalUnidades+=productosIndex[material].unidades - productosIndex[material].aplicado;
            TotalSubUnidades+=productosIndex[material].subUnidades - productosIndex[material].aplicado;
            lista= {
                ...lista,
                [material]:true
            }
...
const verificarBeneficios=(cliente:TCliente, grupoBeneficios:TPromoOngoingGrupoBeneficios[], productosPedidoIndex :TProductosPedidoIndex): TPromoOngoingGrupoBeneficios[] =>
{
    ...
    grupoBeneficios.forEach((grupo:TPromoOngoingGrupoBeneficios) => {
        ...
        for ( let secuencia of grupo.secuencias)
        {
            ...
            {
                materiales = secuencia.materialesBeneficio
                .filter((producto:number)=> validarProductoContraPortafolio(producto,cliente.portafolio));
            ...
            if (materiales.length==0) // si no se pudo validar materiales(productos) para una secuencia se descarta el grupo
                {
                    grupoValido=false;
                    break;
                }
