...
export const obtenerlistaPromocionesVigentes = (
...
): TListaPromoOngoingConIndices | undefined => {
	if (!cliente) return;
	...
	cliente.promocionesHabilitadas?.forEach((promo) => {
		if (
			promo.pomocionesDisponibles > 0 &&
			fechaDentroDelRango(
				fDispositivo,
				listaPromociones[promo.idPromocion].inicioVigenciaPromocion,
				listaPromociones[promo.idPromocion].finVigenciaPromocion
			)
		)
			listaPromos = {
				...listaPromos,
 ...
 */
const obtenerProductosDelPedidoIndex = (
