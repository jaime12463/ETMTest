...
Escenario: N°1 – El cliente de contado o crédito formal no tiene pedidos activos para la fecha de entrega y el pedido cumple el mínimo y no excede el máximo. No se valida crédito disponible.
    ...
    Y la suma del monto del pedido a guardar, cuyo tipo de pedido contribuyeAMinimo = true, es mayor o igual a montoVentaMinimo del cliente 
    Y la suma del monto de los productos de contado, cuyo tipo de pedido es valorizado = true,  + compromiso de cobro es menor o igual a montoVentaMaximo
...
Escenario: N°2 – El cliente de crédito informal no tiene pedidos activos para la fecha de entrega y no excede el límite de crédito y el pedido cumple el mínimo y no excede el máximo
    Dado que el cliente no tiene pedidos en estado Activo para la misma fecha de entrega del pedido a guardar
    Y la suma del monto del pedido a guardar, cuyo tipo de pedido contribuyeAMinimo = true, es mayor o igual montoVentaMinimo del cliente 
    Y la suma del monto de los productos de contado, cuyo tipo de pedido es valorizado = true,  + compromiso de cobro es menor o igual a montoVentaMaximo
...
Escenario: N°3 – El total del pedido no cumple con el pedido mínimo 
    Dado que el cliente no tiene pedidos en estado Activo para la misma fecha de entrega del pedido a guardar
     Y la suma del monto del pedido a guardar, cuyo tipo de pedido contribuyeAMinimo = true, es menor al montoVentaMinimo del cliente 
    Cuando guardo el pedido
...
Escenario: N°4 – El pedido de contado excede con el pedido máximo 
    Dado que la suma del monto de los productos de contado, cuyo tipo de pedido es valorizado = true,  + compromiso de cobro es mayor a montoVentaMaximo
    Y la condición de crédito del cliente es igual crédito informal 
    Cuando guardo el pedido
